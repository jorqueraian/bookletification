...
input_pdf = r"C:/Users/jorqu/Downloads/The Metamorphosis.pdf"
page_scaling = 1
...
while sheets_per_signature > 2:
    remainder = (num_pages+4) % (sheets_per_signature*4)
    num_signatures = int((num_pages+4)//(sheets_per_signature*4))
    if remainder > 0:
        num_signatures += 1
...
sheets_per_signature = int(input("Please select the desired number of sheets per signatures: "))
remainder = (num_pages+4) % (sheets_per_signature*4)
num_signatures = int((num_pages+4)//(sheets_per_signature*4))
if remainder > 0:
    num_signatures += 1
...
reader_pages = ([None]*2) + list(reader.pages) + ([None]*2)  + ([None]*(total_pages_in_book-(num_pages+4)))
...
input_page_width = int(reader_pages[5].mediabox.width)
input_page_height = int(reader_pages[5].mediabox.height)
...
for sig in range(num_signatures):
    ...
    for counter, i in enumerate(range(pages_per_signature//2-1,-1,-1)):
        ...
        scale_factor = (page_height/2)/input_page_width
        ...
        if page_1 is not None:
            ...
            temp_page.add_transformation(Transformation().scale(scale_factor,scale_factor).rotate(-90).translate(topbottom_margins,page_height))#.translate(0, page_height))
            translated_page.merge_page(temp_page)
        if page_2 is not None:
            temp_page = PageObject.create_blank_page(None, page_width, page_height)
            temp_page.merge_page(page_2)
            temp_page.add_transformation(Transformation().scale(scale_factor,scale_factor).rotate(-90).translate(topbottom_margins,page_height/2))
            translated_page.merge_page(temp_page)
